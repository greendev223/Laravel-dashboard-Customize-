...
class ReportController extends Controller
{
        ...
        if ((Auth::user()->can('Manage Purchases') || Auth::user()->can('Manage Sales')) && $request->ajax()) {
            ...
            foreach ($invoices->orderBy('id', 'DESC')->get() as $key => $invoice) {
                ...
                if ($data['vendors'] == 1) {
                    ...
                    $invoicearray[$key]['invoice_id'] = '<span class="btn-shadow btn-info">
                    <a class="text-white" href="#" data-ajax-popup="true" data-title="' . __('Purchase Invoice') . '" data-size="lg" data-url="' . route('show.purchase.invoice', $invoice->id) . '" >' . Auth::user()->purchaseInvoiceNumberFormat($invoice->invoice_id) . '</a>
                      </span>  ';
                    $invoicearray[$key]['action']     = '

                    <div class="action-btn btn-dark ms-2">
                    <a href="' . route('get.purchased.invoice', Crypt::encrypt($invoice->id)) . '" target="_blank" class="mx-3 btn btn-sm d-inline-flex align-items-center " title="' . __('Download') . '"><i class="ti ti-arrow-bar-to-down text-white"></i></a>
                    </div>

                    <div class="action-btn btn-primary ms-2">
                    <a href="' . route('purchase.link.copy', Crypt::encrypt($invoice->id)) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center copy_link" title="' . __('Copy Link') . '"><i class="ti ti-link text-white"></i></a>
                    </div>
                    
                    <div class="action-btn btn-info ms-2">
                    <a href="' . route('edit.purchase.invoice', $invoice->id) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center"  title="' . __('Edit') . '"> <i class="ti ti-edit text-white" title="Edit"></i></a>
                    </div>
                  
                    
                    <div class="action-btn btn-warning ms-2">
                    <a href="#" data-ajax-popup="true" data-title="' . __('Purchase Invoice') . '" data-size="lg" data-url="' . route('show.purchase.invoice', $invoice->id) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center" title="' . __('Show') . '"><i class="ti ti-eye"></i></a>
                    </div>
                    
                    <div class="action-btn bg-danger ms-2">
                    <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center delete-icon" data-toggle="tooltip" title="' . __('Delete') . '" data-confirm="' . __("Are You Sure?") . '|' . __("This action can not be undone. Do you want to continue?") . '" data-confirm-yes=document.getElementById("' . $model_delete_id . '").submit();>
                    ...
                    <div class="action-btn btn-dark ms-2">
                    <a href="' . route('get.sales.invoice', Crypt::encrypt($invoice->id)) . '" target="_blank" class="mx-3 btn btn-sm d-inline-flex align-items-center"><i class="ti ti-arrow-bar-to-down text-white"></i></a>
                    </div>
 
                    <div class="action-btn btn-primary ms-2">
                    <a href="' . route('sale.link.copy', Crypt::encrypt($invoice->id)) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center copy_link_sale" title="' . __('Copy Link') . '"><i class="ti ti-link text-white"></i></a>
                    </div>
 
                     
                    <div class="action-btn btn-info ms-2">
                    <a href="' . route('edit.sale.invoice', $invoice->id) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center"  title="' . __('Edit') . '"><i class="ti ti-edit text-white"></i></a>
                    </div>


                    <div class="action-btn btn-warning ms-2">
                    <a href="#" data-ajax-popup="true" data-title="' . __('Sale Invoice') . '" data-size="lg" data-url="' . route('show.sell.invoice', $invoice->id) . '" class="mx-3 btn btn-sm d-inline-flex align-items-center" title="' . __('Copy Link') . '"><i class="ti ti-eye"></i></a>
                    </div>
    
                    <div class="action-btn bg-danger ms-2">
                    <a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center delete-icon" data-toggle="tooltip" data-original-title="' . __('Delete') . '" data-confirm="' . __("Are You Sure?") . '|' . __("This action can not be undone. Do you want to continue?") . '" data-confirm-yes=document.getElementById("' . $model_delete_id . '").submit();>
                        <i class="ti ti-trash text-white"></i>
                    </a>
                    </div>
        ...
        if ($request->ajax()) {
            ...
            if ($productscount > 0) {
                ...
                foreach ($productObj->get() as $key => $product) {
                    ...
                    $quantity_html = '<span class=" badge rounded bg-' . $class . '">' . $product_quantity . '</span>';
    ...
    public function purchaseLink($id, Request $request)
    {
        ...
        if (!empty($purchase)) {
            ...
            $purchases = $purchase->itemsArray();
            return view('purchases.purchase_invoice', compact('purchases', 'details', 'user'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function saleLink($id, Request $request)
    {
        ...
        if (!empty($sell)) {
            ...
            return view('sales.sale_invoice', compact('sales', 'details', 'user'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
