    ...
    public static function getProductPurchaseTaxAnalysis(array $data)
    {
        ...
        if ($purchasedCollection->count() > 0) {
	        ...
	        foreach ($purchasedCollection as $counter => $pc) {
	            ...
	            $purchasedArray['invoice_id'] = '<a href="#" data-ajax-popup="true" data-title="' . __('Purchase Invoice') . '" data-size="lg" data-url="' . route('show.purchase.invoice', $purchasedArray['id']) . '" class="badge badge-info">' . Auth::user()->purchaseInvoiceNumberFormat($purchasedArray['invoice_id']) . '</a>';
    ...
    public static function getProductSaleTaxAnalysis(array $data)
    {
        ...
        if ($saledCollection->count() > 0) {
	        ...
	        foreach ($saledCollection as $counter => $pc) {
	            ...
	            $saledArray['invoice_id'] = '<a href="#" data-ajax-popup="true" data-title="' . __('Sale Invoice') . '" data-size="lg" data-url="' . route('show.sell.invoice', $saledArray['id']) . '" class="badge badge-info">' . Auth::user()->sellInvoiceNumberFormat($saledArray['invoice_id']) . '</a>';
