<form class="pl-3 pr-3" method="post" action="{{ route('test.email.send') }}" id="test_email">
    ...
    <div class="form-group">
        ...
        <button class="btn btn-primary btn-sm rounded-pill float-left" type="submit">{{ __('Send Test Mail') }}</button>
        <label id="email_sending" class="float-left" style="display: none;"><i class="fas fa-clock"></i> {{ __('Sending ...') }} </label>
    </div>
</form>
