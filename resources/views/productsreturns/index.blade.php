...
@section('action-btn')
    ...
    <a href="{{ route('productsreturns.export') }}" data-bs-toggle="tooltip" class="btn btn-sm btn-primary btn-icon" title="{{ __('Export') }}">
        <i class="ti ti-file-export text-white"></i> 
    </a>

    @can('Create Returns')
        <a href="{{ route('productsreturn.create') }}" data-bs-toggle="tooltip" title="{{ __('Add Return') }}" class="btn btn-sm btn-primary btn-icon m-1">
                    ...
                    <div class="table-responsive">
                        <table class="table" id="pc-dt-simple" role="grid">
                            ...
                            <tr>
                                ...
                                <th style="text-right" width="117px">{{ __('Action') }}</th>
                            </tr>
                            </thead>
                            <tbody>
                            @foreach ($returns as $key => $return)
                                <tr>
                                    ...
                                    <td>{{ $return->vendor != null ? ucfirst($return->vendor->name) : __('Walk-in Vendor') }}</td>
                                    <td>{{ $return->customer != null ? ucfirst($return->customer->name) : __('Walk-in Customer') }}</td>
                                    <td>{{ Auth::user()->priceFormat($return->getTotal()) }}</td>
                                    <td class="pull-right text-right">
                                        @can('Edit Returns')
                                        <div class="action-btn btn-info ms-2">
                                            <a href="{{route('productsreturn.edit', $return->id)}}" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-bs-toggle="tooltip" data-title="{{ __('Edit return') }}" title="{{ __('Edit') }}"><i class="ti ti-pencil text-white"></i></a>
                                        </div>
                                        @endcan
                                        @can('Delete Returns')
                                        <div class="action-btn bg-danger ms-2">
                                            <a href="#" class="bs-pass-para mx-3 btn btn-sm d-inline-flex align-items-center" data-toggle="sweet-alert"
                                                ...
                                                data-confirm-yes="delete-form-{{$return->id}}"  data-bs-toggle="tooltip"
                                                title="{{ __('Delete') }}"><i class="ti ti-trash text-white"></i>
                                            </a>
                                        </a>
