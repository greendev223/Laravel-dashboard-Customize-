                            ...
                            <div class="col-12">
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="ti ti-search text-white"></i></span>
                                    </div>
                            ...
                            <div class="card card-body carttable cart-product-list" id="carthtml" style="margin-top: 5px;margin-bottom: 3px;">
                                ...
                                @if(session($lastsegment) && !empty(session($lastsegment)) && count(session($lastsegment)) > 0)
                                    @foreach(session($lastsegment) as $id => $details)
                                        ...
                                        @endphp
                                        <div class="car-sub row mt-3" data-product-id="{{$id}}" id="product-id-{{$id}}">
                                            <div class="col-sm-2">
                                            ...
                                            <div class="col-sm-10">
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-5">
                                                      <span class="price">{{ Auth::user()->priceFormat($details['price']) }}</span>
                                                    </div>
                                                    <div class="col-sm-5 mt-2">
                                                        <span class="quantity buttons_added">
                                                            <input type="button" value="-" class="minus">
                                                            <input type="number" step="1" min="1" name="quantity" style="width: 50px"
                                                    ...
                                                    <div class="col-sm-2 mt-2">
                                                        <a href="#" class="action-btn bg-danger bs-pass-para" data-confirm="{{__("Are You Sure?")}}" data-text="{{__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="delete-form-{{ $id }}" title="{{ __('Delete') }}" data-id="{{ $id }}">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="tab-content  btn-empty text-center">
                                                <a href="#" class="btn btn-danger " data-toggle="tooltip" data-original-title="{{ __('Empty Cart') }}"
            ...
            <div class="modal-dialog modal-dialog-centered" >
                <div class="modal-content">
                    <div class="modal-body">
                        <span class="select-warning border border-warning">{{  __('Please select Branch and Cash Register.') }}</span>

                        <div class="branch-warning warning alert alert-warning alert-dismissible fade show mt-3" role="alert">
                            <span class="alert-icon"><i class="fa fa-exclamation-triangle"></i></span>
                            <span class="alert-text">
                                <strong> {{ __('Please add some') }} <a href="{{ route('branches.index') }}">{{ __('Branches') }}</a> {{ (' and ') }} <a href="{{ route('cashregisters.index') }}">{{ __('Cash Registers') }}</a></strong>
                            </span>
                        </div>
                    ...
                    success: function (data) {
                        ...
                        if (data.code == '200') {
                            ...
                            $.each(data.product, function (key, value) {
                                sum += value.subtotal;                                
                                $('#product-id-' + value.id + ' .subtotal').text(addCommas(value.subtotal));                                
                                var fdsf = $('#product-id-' + value.id).find('input[name="quantity"]').removeAttr('value').val(value.quantity);
                            });
